...
//     expect(after).toEqual(before) as belt-and-braces against any
//     silent mutation.

import { describe, expect, it, vi } from "vitest";

// Per-process stable UUID-shaped actor id. The repository dispatcher
// at src/episodes/repository.ts selects either the local or Supabase
// adapter depending on env vars; the Supabase adapter writes through
// to PostgreSQL where the actors table's id column is `uuid primary
// key` (see scripts/migrations/001-episodes.sql). A typed UUID
// literal works in both bindings.
...
// override band. Mirrors the lens-test fixture shape.
...
describe("GET /api/episodes/[id]/counterfactual-budget", () => {
  it("preserves episode revision \u2014 broadened AGENTS.md derived-views rule", async () => {
    ...
    // adapter returns a structuredClone on read, so before/after are
    // independent value fetches against the live store.
    const before = await episodeRepository.getOwned(TEST_ACTOR_ID, fixture.id);
    expect(before).toBeDefined();

    // Act: directly invoke the route handler with a valid band.
    const request = new Request(
      `http://localhost/api/episodes/${fixture.id}/counterfactual-budget?band=under-1k`,
    );
  ...
  it("returns 400 for an invalid budget band", async () => {
    const fixture = makeReadyFixture();
    await episodeRepository.create(fixture);

    const request = new Request(
      `http://localhost/api/episodes/${fixture.id}/counterfactual-budget?band=free`,
    );
    const context = { params: Promise.resolve({ id: fixture.id }) };
    const response = await GET(request, context);
    ...
    expect(body.error).toContain("Invalid band");
  });

  it("evaluates an alternative budget band and returns a payload", async () => {
    const fixture = makeReadyFixture();
    await episodeRepository.create(fixture);

    // Successful happy-path against a different band than the
    // fixture's stated budget (1k-2k). Override to 3k-plus so the
    // helper exercises the upper ceiling path of budgetVerdict.
    const request = new Request(
      `http://localhost/api/episodes/${fixture.id}/counterfactual-budget?band=3k-plus`,
    );
    const context = { params: Promise.resolve({ id: fixture.id }) };
    const response = await GET(request, context);
