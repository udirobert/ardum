...
// show evidence as hairline-divided rows inside one bordered list: title,
// body, source badge, and a provenance line. Quiet and information-first —
...
export default function EvidenceCards({
  ...
  return (
    <div className="space-y-2" data-testid="evidence-cards">
      ...
      <div className="border border-[color:var(--hairline)] rounded-sm divide-y divide-[color:var(--hairline)] overflow-hidden">
        {cards.map((card, i) => {
          const refreshed = formatRefreshed(card.refreshedAt);
          const badgeStyle = BADGE_STYLES[card.badge];
          return (
            <div key={`${card.title}-${i}`} className="px-3 py-2.5">
              ...
              <p className="pt-2 pb-1 text-[12.5px] leading-relaxed text-[color:var(--muted)]">
                {card.body}
              </p>
              <div>
                <span
